...
struct BST {
		...
		int calculateTreeHeight(BTNode*);
		bool display(int, int);
		void display2(int, ostream&);
		void ascPrint(BTNode*, ostream&);
		void descPrint(BTNode*, ostream&);




		bool printLevelNodes(); // print nodes in each level
		bool CloneSubtree(BST, type);	// clone a subtree from a BST
		bool CloneSubTree2(BTNode *, type); // recursive function for CloneSubtree()
		void CloneSubTree3(BTNode* cur);	// print if item found	
		bool PrintPath();	// print path from root to leaf
		void PrintExternalPath(BTNode*, int[], int);// print path from root to leaf
